<?php
    ...
    function subscriber_list_view ($table) {
        $s = render_button('Add Subscriber', 'subscribers.php?action=add') . '<br><br>';
        $s .= '<table>';
        $s .= '<tr><th>Name</th><th>Email</th></tr>';
        foreach($table as $row) {
            $edit = render_link($row[1], "subscribers.php?id=$row[0]&action=edit");
            $email = $row[2];
            $delete = render_link("delete", "subscribers.php?id=$row[0]&action=delete");
